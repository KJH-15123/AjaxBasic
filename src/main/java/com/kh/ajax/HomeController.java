...
public class HomeController {
	...
	*/
	...
	public String ajax(String name,int age){
		...
		//앞뒤로 /WEB-INF/views/ 와 .jsp 를 붙여서 요청해준다.
		//문자열 자체로(데이터자체로) 응답하고자 한다면 해당 메소드에
		//@ResponseBody 어노테이션을 부여해야한다.
		//인코딩 처리 및 응답데이터 형식은 Mapping어노테이션에 같이 작성한다.
		//produces 속성에 작성하기
	...
	public String test(String menu,int price) {
		
		System.out.println(menu+" "+price);
		
		String responseStr = menu+" 메뉴는 "+price+"원 입니다.";
	...
	public String selectMember(int userNo) {
		...
		//와 같이 추가해야한다.
		return jobj.toJSONString();//json객체를 문자열로 변환해주는 메소드
	}
	
	*/
	
	//위 작업을 GSON 을 이용하여 쉽게 해보기 
	@ResponseBody
	@RequestMapping(value="/select.me",produces = "application/json;charset=UTF-8")
	public Member selectMember(int userNo) {
		...
		Member m = Member.builder().name("김유저")
								   ...
								   .email("user01@naver.com")
								   .build();
		//Gson라이브러리에서 제공하는 메소드 
		//객체를 json화 시켜주는 메소드 : toJson(obj)
		System.out.println(new Gson().toJson(m));
		
		
		return m;//json객체를 문자열로 변환해주는 메소드
	}

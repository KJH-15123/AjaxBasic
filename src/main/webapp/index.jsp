...
<body>
	...
	<p>
		개요 * <br> Asynchronous JavaScript And XML 의 약자로 <br> 서버로부터
		데이터를 가져와 전체 페이지를 새로고침하지 않고 일부만 로드할 수 있게 하는 기법 <br> 기존에 사용하던 a태그
		요청 및 form 요청등은 동기식 요청이다. <br> 동기식 요청은 응답페이지가 돌아와야 다른 작업을 수행 가능 <br>
		비동기식 요청을 보내기 위해 AJAX 기술을 이용한다. <br> *동기식 : 요청 처리 후 그에 해당하는 응답페이지가
		돌아와야 다음 작업 수행 <br> 만약 서버에서 호출된 결과까지의 시간이 지연된다면 계속 대기상태 <br>
		전체 페이지가 리로드 된다 (새로고침) <br> <br> *비동기식 : 현재 페이지를 그대로 유지하며 중간에
		추가적인 요청 작업을 할 수 있다. <br> 다른 페이지로 넘어가는것이 아니라 데이터 통신을 처리하는 것 <br>
		요청을 보내놓고 그에 해당하는 응답이 돌아오기전에도 다른 작업을 수행할 수 있다. <br> ex)아이디
		중복체크,검색어 자동완성 등등 <br> <br> 비동기식 요청의 단점 <br> 현재 페이지에
		지속적인 리소스가 쌓여 페이지 로드가 느려질 수 있음 <br> 요청 후 돌아온 응답에 대해 새로운 요소를 만들어
		보여주는 작업능력 필요(DOM) <br>
	</p>
	...
	<script>
		//위 메뉴와 가격 데이터를 입력후 버튼을 누르면 결과div에  OO 메뉴의 가격은 OOO원 입니다 라는 메시지가 출력되도록
		//작성해보기 
		//요청 매핑주소와 메소드명은 자유롭게하고 post요청은 위에 작성한 필기 참조하여 속성넣기
	...
	<h3>3.조회요청 후 조회된 데이터 여러개를 받아 출력해보기</h3>
	조회할 회원 번호 :
